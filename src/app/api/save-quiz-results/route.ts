...
export async function POST(request: NextRequest) {
  try {
    ...
    const quizAnswers: QuizAnswer[] = answers.map((answer: any) => ({
      ...
      selected_answer: answer.selectedAnswer || null, // Allow null for text-based answers
      ...
      confidence_percentage: answer.confidencePercentage || 0,
      model_confidence_percent: answer.confidencePercentage || 0,
      confidence_capture_mode: answer.confidenceCaptureMode || 'auto_after_delay',
      confidence_capture_delay_sec: answer.confidenceCaptureDelaySec || 5,
      answer_type: 'text',
      user_id: userId || null
    }));
